...
class PlayState extends MusicBeatState
{
	...
	//rating stuff on the side
	var ratingsCounter:FlxTypedGroup<FlxText>;
	var ratingNames = ['Sicks', 'Goods', 'Bads', 'Shits'];
	var curSongRatings = [0, 0, 0, 0];
	...
	override public function create()
	{
				...
				case 'snow':
					{
						...
						var sSKY:FlxSprite = new FlxSprite(-222, -16 + 150).loadGraphic(Paths.image('STAGES/snow/SKY', 'exe'));
						sSKY.antialiasing = true;
						sSKY.scrollFactor.set(1, 1);
						sSKY.active = false;
						add(sSKY);

						var hills:FlxSprite = new FlxSprite(-264, -156 + 150).loadGraphic(Paths.image('STAGES/snow/HILLS', 'exe'));
						...
						var bg2:FlxSprite = new FlxSprite(-345, -289 + 170).loadGraphic(Paths.image('STAGES/snow/trees', 'exe'));
						bg2.updateHitbox();
						...
						var bg:FlxSprite = new FlxSprite(-297, -246 + 150).loadGraphic(Paths.image('STAGES/snow/FLOOR1', 'exe'));
						bg.antialiasing = true;
						bg.scrollFactor.set(1.3, 1);
						bg.active = false;
						add(bg);

						var mikoTree:FlxSprite = new FlxSprite(-225, -219 + 150).loadGraphic(Paths.image('STAGES/snow/mikoTree', 'exe'));
						mikoTree.updateHitbox();
						mikoTree.antialiasing = true;
						mikoTree.scrollFactor.set(1.32, 1);
						mikoTree.active = false;

						add(mikoTree);

						var matsuri:FlxSprite = new FlxSprite(185 + 100, -350 + 150).loadGraphic(Paths.image('STAGES/snow/matsuri', 'exe'));
						matsuri.updateHitbox();
						matsuri.antialiasing = true;
						matsuri.scrollFactor.set(1.36, 1);
						matsuri.active = false;

						add(matsuri);

						var sticklol:FlxSprite = new FlxSprite(-100, 50);
						sticklol.frames = Paths.getSparrowAtlas('STAGES/snow/TailsSpikeAnimated', 'exe');
				...
				case 'painted':
					{
						defaultCamZoom = 0.7;
						curStage = 'painted';
						var bg:FlxSprite = new FlxSprite(-300, -200).loadGraphic(Paths.image('STAGES/inaStage/inaBG', 'exe'));
						...
						var stageFront:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('STAGES/inaStage/inaFG', 'exe'));
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
		{
			...
			songName = new FlxText(songPosBG.x, songPosBG.y, songPosBG.width, SONG.song + " (0:00)", 16);
			if (PlayStateChangeables.useDownscroll)
				songName.y -= 3;
		...
		//add ratings n shit to the side lol
		ratingsCounter = new FlxTypedGroup<FlxText>();

		var ratingFontSize = 20;
		var initialRatingY = Std.int((FlxG.height / 2) - ((ratingFontSize * ratingNames.length) / 2));

		for (i in 0...ratingNames.length)
		{
			var sex = new FlxText(0, initialRatingY + (ratingFontSize * i), 0, ratingNames[i]);
			sex.setFormat(Paths.font("vcr.ttf"), ratingFontSize, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			sex.cameras = [camHUD];
			ratingsCounter.add(sex);
			add(sex);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (startingSong)
		{
			if (startedCountdown)
			{
				...
				if (Conductor.songPosition >= 0)
					if (curSong.toLowerCase() != 'too-seiso' || curSong.toLowerCase() != 'cycles')
					{
						startSong();
					}
			}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					//cam shit sonic exe spooky shit
					switch (singData)
					{
						case 0:
							camX = -15;
		...
		//Do the funny time thing
		//totally didnt copy this from psych
	...
	override function stepHit()
	{
		...
		//the fuckin sonic exe stuff

		if (FlxG.save.data.jumpscares == null) FlxG.save.data.jumpscares = true; //eh lets just assume they want it lol

		//SUBTRACTED EVEERY MY 20
		if (SONG.song.toLowerCase() == 'too-seiso')
		{
			switch (curStep)
			{
				case 765:
				...
				case 1305:
					...
					//remove this when sui anim finally gets added
					fuckingThing = new FlxSprite(0, 0).loadGraphic(Paths.image('suiSTFU', 'exe'));
		...
		if (curStage == 'snow' && curStep != stepOfLast && FlxG.save.data.jumpscares)
			{
				var staticSteps:Array<Int> = [27, 130, 265, 450, 645, 800, 855, 889, 921, 938, 981, 1030, 1065, 1105, 1123, 1245, 1345, 1432, 1454, 1495, 1521, 1558, 1578,1599, 1618, 1647, 1657,1692, 1713,1738, 1747,1761, 1785, 1806, 1816,1832,1849,1868,1887, 1909];

				for (i in staticSteps)
				{
					if (curStep == i) doStaticSign(0);
				}
	...
	function doStaticSign(lestatic:Int = 0, leopa:Bool = true)
	{
		...
		daStatic.frames = Paths.getSparrowAtlas('STAGES/snow/daSTAT', 'exe');
		...
		FlxG.sound.play(Paths.sound('too-seiso/staticBUZZ', 'exe'));
	...
	function doSimpleJump()
	{
		...
		var simplejump:FlxSprite = new FlxSprite().loadGraphic(Paths.image('STAGES/snow/shitpissandcum', 'exe'));
		...
		FlxG.sound.play(Paths.sound('too-seiso/shitpant', 'exe'), 1);
	...
	function doJumpscare()
	{
		...
		daJumpscare.frames = Paths.getSparrowAtlas('STAGES/snow/sonicJUMPSCARE', 'exe');
		daJumpscare.animation.addByPrefix('jump', 'sonicSPOOK', 24, false);
		...
		FlxG.sound.play(Paths.sound('too-seiso/JUMPSCARE_begin', 'exe'), 1);
		FlxG.sound.play(Paths.sound('too-seiso/JUMPSCARE_bgnoise', 'exe'), 1);
