...
class Note extends FlxSprite
{
	...
	public var noteSinger:Int = 0;

	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?noteType:Int = 0, ?noteSinger:Int)
	{
		super();
		...
		else
		{
			...
			switch (noteTypeCheck)
			{
				...
				default:
					...
					switch (noteType)
					{
						case 3: //phantom notes
						{
							frames = Paths.getSparrowAtlas('notes/PhantomNote', 'exe');
							noteSuffix = ' withered';
						}
						case 2: //static notes
						{
							frames = Paths.getSparrowAtlas('notes/staticNotes', 'exe');
							noteSuffix = ' static';

							var fuckingSussy = Paths.getSparrowAtlas('notes/staticNotes', 'exe');
					...
					for (i in 0...4)
					{
						animation.addByPrefix(dataColor[i] + 'Scroll', dataColor[i] + noteSuffix); // Normal notes
